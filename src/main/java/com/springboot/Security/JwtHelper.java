...
//    private static final long JWT_TOKEN_VALIDITY = 9000;
	//private static final long JWT_TOKEN_VALIDITY = 20;
	private static final long JWT_TOKEN_VALIDITY = 15 * 60 * 1000; // 15 minutes
    ...
    private String doGenerateToken(Map<String , Object> claims, Authentication authentication) {
        String role = authentication.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toSet()).iterator().next();
